...
<script>
  ...
  export default {
    ...
    asyncData({ route }) {
      return axios.get(`http://cms.walter.ba/wp-json/wp/v2/careers/${route.params.id}?_embed`).then((response) => {
        return {
          page: response.data,
    ...
    methods: {
      ...
      getItems() {
        axios.get('http://cms.walter.ba/wp-json/wp/v2/careers?_embed').then((response) => {
